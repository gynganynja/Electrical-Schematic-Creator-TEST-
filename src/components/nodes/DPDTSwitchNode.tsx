...
export function DPDTSwitchNode({ id, data, selected }: any) {
    ...
    return (
        <div className={`bg-slate-900 border-2 rounded-lg w-32 flex flex-col items-center shadow-lg relative transition-all ${selected ? 'border-blue-500 shadow-[0_0_15px_rgba(59,130,246,0.5)]' : 'border-slate-700 hover:border-slate-500'}`}>
            ...
            {/* Pole A handles */}
            <Handle type="target" position={Position.Left} id="in_a"
                className="!w-3 !h-3 !bg-amber-400 !border-2 !border-white" style={{ top: '25%' }} />
            <Handle type="source" position={Position.Right} id="out_a_nc"
                className="!w-3 !h-3 !bg-emerald-400 !border-2 !border-white" style={{ top: '15%' }} />
            <Handle type="source" position={Position.Right} id="out_a_no"
                className="!w-3 !h-3 !bg-sky-400 !border-2 !border-white" style={{ top: '35%' }} />

            {/* Pole B handles */}
            <Handle type="target" position={Position.Left} id="in_b"
                className="!w-3 !h-3 !bg-amber-400 !border-2 !border-white" style={{ top: '65%' }} />
            <Handle type="source" position={Position.Right} id="out_b_nc"
                className="!w-3 !h-3 !bg-emerald-400 !border-2 !border-white" style={{ top: '55%' }} />
            <Handle type="source" position={Position.Right} id="out_b_no"
                className="!w-3 !h-3 !bg-sky-400 !border-2 !border-white" style={{ top: '75%' }} />
        </div>
    );
}
